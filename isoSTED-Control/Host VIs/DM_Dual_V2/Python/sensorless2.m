...
% run this file on its containing directory (press F5)
...
try
    while run
        dset = hdf5read(logfile, sprintf('/msvst/denoised/%09d', i))';
...
try
	...
	modal_func = hdf5read(logfile, '/modal/name');
...
log_x = hdf5read(logfile, 'modal/log_x')';
log_y = hdf5read(logfile, 'modal/log_y')';
assert(numel(log_y) == size(stacks, 2) - 2);
assert(size(log_x, 2) == size(stacks, 2) - 2);
blob.log_x = log_x;
blob.log_y = log_y;
