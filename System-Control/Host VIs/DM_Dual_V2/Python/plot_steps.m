...
plot_mode(priv, 1);
    ...
    function plot_mode(mypriv, myind)
        ...
        if ~isempty(mypriv.ring0)
            ...
            subplot(2, 2, 2);
            imagesc(reshape(mypriv.rings(:, myind), size(mypriv.ring0)));
            axis equal;
            axis tight;
            axis off;
            colorbar();
            ...
            grid on;
            hold on;
            plot(myind, mypriv.log_y(myind), 'rx');
            hold off;
            
            if ~isempty(mypriv.metric_stacks)
                subplot(2, 2, 4);
                imagesc(reshape(mypriv.metric_stacks(:, myind), ...
                    size(mypriv.stack0)));
                axis equal;
                axis tight;
                axis off;
        ...
        elseif ~isempty(mypriv.metric_FM)
            sfigure(4);
            clf();
            ...
            if myind > 1
                ...
                subplot(2, 2, 3);
                imagesc(fftshift(...
                    reshape(mypriv.metric_FM(:, myind - 1), ...
                    size(mypriv.stack0))));
                axis equal;
                axis tight;
                axis off;
